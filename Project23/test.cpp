...
protected:
	void SetUp() override {
		...
		EXPECT_CALL(CUSTOMER, getEmail).WillRepeatedly(testing::Return(""));

		EXPECT_CALL(CUSTOMER_WITH_MAIL, getEmail).WillRepeatedly(testing::Return("test@test.com"));
	}
...
public:
	...
	// Customer CUSTOMER{ "Fake name", "010-1234-4567" };
	// Customer CUSTOMER_WITH_MAIL{ "Fake name", "010-1234-4567", "test@test.com"};
...
TEST_F(BookingItem, 현재날짜가일요일인경우예약불가예외처리) {
	...
	EXPECT_CALL(mockScheduler, getNow).WillRepeatedly(testing::Return(mktime(&SUNDAY_ON_THE_HOUR)));
...
TEST_F(BookingItem, 현재날짜가일요일이아닌경우예약가능) {
	TestableBookingScheduler mockScheduler{ CAPACITY_PER_HOUR };
	EXPECT_CALL(mockScheduler, getNow).WillRepeatedly(testing::Return(mktime(&MONDAY_ON_THE_HOUR)));
